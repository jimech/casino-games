...
- Tournament service defines active/upcoming events, stores entries in memory or Prisma, debits entry fees through wallet ledger entries, prevents duplicate user entry, and computes leaderboards from settled post-entry round records only. Tournament scores are signed `payout - stake` values with deterministic tie-breaks.
