...
- Provably fair seed lifecycle records are committed before outcome generation in memory mode. Roulette and slots reveal immediately after settlement; crash stores a commitment at round start and reveals the server seed only when the round settles.
